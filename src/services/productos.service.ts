...
import type { ProductResponse, CategoryProductResponse, Product } from "../interfaces/Productos.interface"
import type { VarianteDetalleModel } from "../interfaces/VarianteDetalleModel.interface"
 ...
 * Obtener productos destacados con paginación
 */
export const getDestacados = async (page = 1, limit = 12): Promise<ProductResponse> => {
  try {
 ...
 * @param limit - Cantidad de productos por página (default: 12)
 */
export const searchProductos = async (query: string, page = 1, limit = 12): Promise<ProductResponse> => {
  try {

    const url =`${API_BASE_URL}/variantes/buscar?q=${query}&page=${page}&limit=${limit}`;
    const response = await fetch( url    )
 ...
 */
export const getNovedades = async (page = 1, limit = 12): Promise<ProductResponse> => {
  try {
 ...
 */
export const getByCategoria = async (slug: string, page = 1, limit = 12): Promise<CategoryProductResponse> => {
  try {
...
export const filterProducts = (
  ...
  if (filters.search && filters.search.trim() !== "") {
    ...
    filtered = filtered.filter(
      (p) => p.name.toLowerCase().includes(searchLower) || p.slug.toLowerCase().includes(searchLower),
    )
  }
 ...
 */
export const getAllCategoriesWithProducts = async (categorySlugs: string[]): Promise<Map<string, Product[]>> => {
